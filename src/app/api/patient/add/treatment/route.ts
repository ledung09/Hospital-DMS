...
export async function POST(req: Request){
  ...
  if (!isStartDateBeforeEndDate(body.admissiontime, body.starttime)) {
    return Response.json({ res: "fail", warning: "Admission timestamp is after Start timestamp!" }, { status: 200 });
  }

  if (!isStartDateBeforeEndDate(body.starttime, body.endtime)) {
    return Response.json({ res: "fail", warning: "Start timestamp is after End timestamp!" }, { status: 200 });
  }
